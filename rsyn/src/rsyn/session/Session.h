...
struct SessionData {
	...
	robin_hood::unordered_map<std::string, ServiceInstantiatonFunction> clsServiceInstanciationFunctions;
	robin_hood::unordered_map<std::string, Service *> clsRunningServices;
	...
	// Generic functions used to instantiate optimizers by name.
	robin_hood::unordered_map<std::string, ReaderInstantiatonFunction> clsReaders;
...
private:
	...
	static void listService(std::ostream & out = std::cout) {
		...
		for (auto srv : sessionData->clsServiceInstanciationFunctions) {
...
private:
	static void listReader(std::ostream & out = std::cout) {
		...
		for(auto reader : sessionData->clsReaders) {
			out<<"\t"<<reader.first<<"\n";
		} // end for 
		out<<"--------------------------------------\n";
	} /// end method
